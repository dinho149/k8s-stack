#!/usr/bin/env bash
# doctor.sh — verify the local toolchain and environment for `make teleport-up`.
...
if [[ -x "$BIN_DIR/tsh" ]]; then ui::status ok "tsh / tctl" "$("$BIN_DIR/tsh" version 2>/dev/null | head -1 | grep -oE 'v[0-9.]+' | head -1) in ./bin"; else ui::status warn "tsh / tctl" "not installed — run: make teleport-tsh"; fi
if command -v claude >/dev/null 2>&1; then
  ui::status ok "claude code" "$(claude --version 2>/dev/null | head -1) — agent can use your subscription (make teleport-agent-cli)"
else ui::status warn "claude code" "not installed — agent needs ANTHROPIC_API_KEY or 'npm i -g @anthropic-ai/claude-code'"; fi
if command -v expect >/dev/null 2>&1; then ui::status ok "expect" "present (headless tsh login: make teleport-login, e2e)"; else ui::status warn "expect" "missing — make teleport-login / e2e need it (brew/apt install expect)"; fi
if command -v mkcert >/dev/null 2>&1; then
  if [[ -f "$(mkcert -CAROOT 2>/dev/null)/rootCA.pem" ]]; then ui::status ok "mkcert" "CA in $(mkcert -CAROOT)$( [[ -f "$LOCAL_TLS_DIR/teleport.crt" ]] && echo ", local proxy certificate present" || echo " — make teleport-tls generates the proxy certificate")"
  else ui::status warn "mkcert" "installed, root CA not created yet — make teleport-tls (asks for your password once)"; fi
else ui::status warn "mkcert" "not installed — browsers will warn about the self-signed proxy cert (brew install mkcert; make teleport-up offers it)"; fi
...
if [[ -d "$REPO_ROOT/.git" ]]; then
  ...
  else ui::status warn "git hooks" "pre-commit hooks not installed — run: make teleport-hooks (CI runs the same checks, so commits without them fail there)"; fi
fi
...
if kind get clusters 2>/dev/null | grep -qx "$KIND_CLUSTER"; then ui::status ok "kind cluster" "$KIND_CLUSTER exists"; else ui::status warn "kind cluster" "$KIND_CLUSTER not created yet (make local)"; fi
port="${PROXY_ADDR##*:}"
...
else ui::status fail "pulumi secrets" "stack $STACK: file backend or default passphrase — run: make teleport-secrets-guard for details"; fail=1; fi
...
if [[ -f "$REPO_ROOT/infra/teleport/Pulumi.$STACK.yaml" ]]; then
  ...
  else ui::status warn "agent auth" "no Claude credentials in stack $STACK — make teleport-claude-token (subscription) or set teleport:chat.anthropicApiKey"; fi
fi
if [[ -f "$REPO_ROOT/infra/teleport/Pulumi.$STACK.yaml" ]] && grep -q 'github:' "$REPO_ROOT/infra/teleport/Pulumi.$STACK.yaml" 2>/dev/null && grep -q 'clientSecret' "$REPO_ROOT/infra/teleport/Pulumi.$STACK.yaml"; then ui::status ok "GitHub SSO" "OAuth app configured for stack $STACK"; else ui::status warn "GitHub SSO" "not configured for stack $STACK — run: make teleport-github-sso (local login still works)"; fi

echo
if (( fail )); then ui::fail "doctor found blocking problems"; exit 1; else ui::ok "all good — next: make teleport-up"; fi
