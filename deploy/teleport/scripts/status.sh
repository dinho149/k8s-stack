...
ui::section "Teleport status  ${UI_DIM}stack=$STACK cluster=$KIND_CLUSTER${UI_RESET}"
if kind get clusters 2>/dev/null | grep -qx "$KIND_CLUSTER"; then ui::status ok "kind" "$KIND_CLUSTER ($($KUBECTL get nodes --no-headers 2>/dev/null | wc -l | tr -d ' ') node)"; else ui::status fail "kind" "cluster $KIND_CLUSTER not found — make local"; exit 0; fi
...
else ui::status warn "pulumi" "stack $STACK not deployed yet — make teleport-up"; fi
...
if [[ -n "$ping" ]] && $KUBECTL -n "$TELEPORT_NAMESPACE" get pods -l app.kubernetes.io/component=auth --no-headers 2>/dev/null | grep -q Running; then
  ...
  ui::kv "pending requests" "$pend  ${UI_DIM}(make teleport-requests)${UI_RESET}"
fi

ui::section "You"
if [[ -x "$BIN_DIR/tsh" ]]; then $TSH status 2>/dev/null | sed 's/^/  /' | head -12 || ui::info "not logged in — make teleport-login"; else ui::info "tsh not installed — make teleport-tsh"; fi
