...
#
# Uses the ci-harness bot identity to issue reset tokens through the Teleport API (tests/teleport/tools/seed-users)
# and stores the credentials in .dogfood/teleport/state/users.json (0600, gitignored). Users already in the file are
...
"$REPO_ROOT/deploy/teleport/scripts/harness-identity.sh"

# The operator creates TeleportUser CRs asynchronously: wait until every requested user exists.
wait_users() {
  local t=0
  until "$REPO_ROOT/deploy/teleport/scripts/tctl.sh" users ls --format=json 2>/dev/null \
