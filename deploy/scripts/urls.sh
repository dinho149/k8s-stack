...
ui::table "WHAT|URL|NOTE" \
  "Teleport web UI|https://$PROXY_ADDR|${TSH_INSECURE_FLAG:+self-signed cert (accept warning)}" \
  ...
  "tsh|./bin/tsh ${TSH_INSECURE_FLAG:+$TSH_INSECURE_FLAG }--proxy $PROXY_ADDR login|or: make login" \
