...
# Uses localhost port-forwards and the stack's secret outputs, like deploy/teleport/scripts/agent-cli.sh.
...
KUBE_CONTEXT="${KUBE_CONTEXT:-kind-${KIND_CLUSTER:-dogfood-local}}"
...
trap cleanup EXIT
$KUBECTL -n teleport-access port-forward svc/access-broker 18381:8081 >"$UI_LOG_DIR/e2e-pf-broker.log" 2>&1 & pids+=($!)
$KUBECTL -n teleport-access port-forward svc/teleport-mcp 18380:8080 >"$UI_LOG_DIR/e2e-pf-mcp.log" 2>&1 & pids+=($!)
for _ in $(seq 1 20); do curl -fs --max-time 2 http://localhost:18381/healthz >/dev/null 2>&1 && curl -s --max-time 2 -o /dev/null http://localhost:18380/mcp 2>/dev/null && break; sleep 1; done
...
code="$(curl -s -o "$UI_LOG_DIR/e2e-broker-approve.log" -w '%{http_code}' --max-time 10 \
  -X POST "http://localhost:18381/v1/requests/$id/approve" \
...
code="$(curl -s -o "$UI_LOG_DIR/e2e-mcp-noassert.log" -w '%{http_code}' --max-time 10 \
  -X POST "http://localhost:18380/mcp" \
