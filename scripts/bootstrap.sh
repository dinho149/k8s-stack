...
: "${STACK_NAME:?}" "${STACK_PROVIDER:?}"
mkdir -p "$ROOT/.stack"
started=$(date +%s)
mode=existing
if [[ "$STACK_PROVIDER" == kind ]]; then
  if ! kind get clusters 2>/dev/null | rg -qx "$STACK_NAME"; then
    mode=cold
    kind create cluster --name "$STACK_NAME" --image kindest/node:v1.34.0 --config "$ROOT/deploy/kind/config.yaml" --wait 0s
  fi
  kind export kubeconfig --name "$STACK_NAME"
...
if [[ "$STACK_PROFILE" == local ]]; then
 ...
 python3 "$ROOT/scripts/tool-routes.py" --domain "${STACK_DOMAIN:-127.0.0.1.nip.io}" | k apply -f -
else
