...
def credentials(create=False):
    ...
    # Teleport access pages: make teleport-portal-forward writes the portal API URL + service token here.
    portal_env = STATE / 'teleport-portal.env'
    ...
    ENV.setdefault('TELEPORT_LOCAL_SUBJECT', 'local-developer')
...
def teleport(command):
    ...
    elif command == 'teleport-portal-forward':
        foreground([script('portal-forward.sh')] + (['--background'] if ENV.get('PORTAL_FORWARD') == 'background' else []))
